...
<html>
  <head>
	...
	<script type="text/javascript">
		var store, colModel, dataGridSystemLog, searchForm, tabPanel, dataModePanel, imageModePanel;
		...
		Ext.onReady(function(){
		        ...
		        items: [{
		            ...
		            items:[{
		                ...
		                items : [{
		                    ...
		                    //style: 'margin:15px 0 0 0',
		                    handler: function() {
								var url = basePath + 'systemLog/export.do?t=' + new Date().getTime();
								if(Ext.getCmp('txtOperateId').getValue().trim() != ""){
									url += '&id=' + Ext.getCmp('txtOperateId').getValue();
								}
								if(Ext.getCmp('comType').getValue() != "" && Ext.getCmp('comType').getValue() > 0){
									url += '&comType=' + Ext.getCmp('comType').getValue();
								}
								if(Ext.getCmp('txtUserName').getValue().trim() != ""){
									url += '&txtUserName=' + Ext.getCmp('txtUserName').getValue();
								}
								if(Ext.getCmp('comRegSource').getValue().trim() != "" && Ext.getCmp('comRegSource').getValue() > 0){
									url += '&comRegSource=' + Ext.getCmp('comRegSource').getValue();
								}
								...
								if(Ext.getCmp('comTerminal').getValue().trim() != "" && Ext.getCmp('comTerminal').getValue() != 0){
									url += '&comTerminal=' + Ext.getCmp('comTerminal').getValue();
								}
			...
			store = new Ext.data.Store({
				...
				　　url:basePath + 'systemLog/systemLogList.do',
				　　method:'GET'
				}),
				remoteSort:true
			});
			...
			store.on('load', function(store){
				dataGridSystemLog.getSelectionModel().selectFirstRow();
				//dataGridSystemLog.setHeight(window.parent.Ext.get('menuPanel').getHeight() - 29);
			});
			window.onresize = function(){
				dataGridSystemLog.setHeight(window.parent.Ext.get('menuPanel').getHeight() - searchHeight - toolbarHeight);
			}
			store.load({params:{start:0, limit:20}});
