	    ...
	    */
	    ...
	    JSONStringObject getJSONObject(Object value, boolean useClassConvert) throws JSONException{
	        ...
	        if (value instanceof Date) {
	        	return new JSONStringObject(JSONObject.quote(DateHelper.dateToString(value)));
			}
	    ...
	    private JSONStringObject reflectObject(Object bean, boolean useClassConvert) {
	        ...
	        //过滤指定的属性
	        HashSet<String> jsonPropsFilterSet = new HashSet<String>();
	        for (int i = 0; i < methods.length; i += 1) {
	            try {
	                Method method = methods[i];
	                String name = method.getName();
	                if(name.equals("getJsonPropsFilter")){
	                	
	                	Object elementObj = method.invoke(bean, null);
	                	if(elementObj!=null&&elementObj.toString().length()>0){
	                		jsonPropsFilterSet = StringUtil.getHashSet(elementObj.toString().split(","));
	                	}
	                	break;
	                }
	                if(name.equals("getFilterType")){
	                	Object filterType = method.invoke(bean, null);
	                	isFilterType = filterType.equals("yes") ? true :false; 
	                }
	            }catch(Exception ex){
	            	ex.printStackTrace(); 
	            }
	        ...
	        for (int i = 0; i < methods.length; i += 1) {
	            try {
	                ...
	                if(name.equals("getJsonPropsFilter") || name.equals("getFilterType")){
	                	continue;
	                }
	                String key = "";
	                ...
	                if (key.length() > 0 &&
	                    ...
	                    if(!useClassConvert && elementObj instanceof Class)
	                        continue;
	                    if(isFilterType){
	                    	if(jsonPropsFilterSet.size()>0&&!jsonPropsFilterSet.contains(key)){
	                    		continue;
	                    	}
	                    }else{
	                    	if(jsonPropsFilterSet.contains(key)){
		                    	continue;
		                    }
	                    }
